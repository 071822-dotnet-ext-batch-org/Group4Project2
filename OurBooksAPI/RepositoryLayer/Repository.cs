    ...
    public class Repository
    {
        public async Task<RegisterAccount> RegisterAccountAsync(Guid userId, string? firstName, string? lastName, string? deliveryAddress, int phone, string? email, string? isAdmin)
        {
            ...
            using SqlCommand command = new SqlCommand($"INSERT INTO Users (UserId, FirstName, LastName, DeliveryAddress, Phone, Email, isAdmin) VALUES ( @UserId, @FirstName, @LastName, @DeliveryAddress, @Phone, @Email, @isAdmin) ", connect);
            {
                ...
                if (ret.Read())
                {
                    RegisterAccount c_info = new RegisterAccount(ret.GetGuid(0), ret.GetString(1), ret.GetString(2), ret.GetString(3), ret.GetInt32(4), ret.GetString(5), ret.GetString(6));
                    connect.Close();
                    return c_info;
